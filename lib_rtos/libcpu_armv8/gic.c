...
struct arm_gic
{
    ...
    rt_uint64_t dist_hw_base;
    rt_uint64_t cpu_hw_base;
};
...
void arm_gic_tri_swi(rt_uint32_t index, int irq)
{
	rt_uint32_t mask = irq % 16;
	mask |= 1<<16;
	GIC_DIST_SOFTINT(_gic_table[index].dist_hw_base) = mask;
}

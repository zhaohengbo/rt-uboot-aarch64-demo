...
static int reserve_rtos(void)
{
	#define TOTAL_RTOS_LENGTH 0x4000
	
	gd->rtos_mem_end = gd->start_addr_sp - 4;
	
	gd->start_addr_sp -= TOTAL_RTOS_LENGTH;
	gd->start_addr_sp &= ~(4096 - 1);
	
	gd->rtos_mem_start = gd->start_addr_sp;
	
	debug("Reserving %dk for rtos at: %08lx\n",
	      TOTAL_RTOS_LENGTH >> 10, gd->start_addr_sp);
	return 0;
}
